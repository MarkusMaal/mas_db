<?php include("../head.php"); ?>
<h3><?php echo ms("Desktop", "Lauaarvuti");?></h3>
<ul>
    <li><?php echo ms("Processor", "Protsessor");?>: AMD Ryzen 7 5700X (<?php echo ms("8c/16t", "8t/16l");?>)</li>
    <li><?php echo ms("Memory", "Mälu");?>: 32GB (<?php echo ms("slots", "pesad");?> A2 &amp; B2, 3200MHz, CL16)</li>
    <li><?php echo ms("Storage", "Andmekandjad");?>: WD Blue 2TB M.2 NVMe SSD + WD Green 240GB M.2 SATA SSD + 2TB Toshiba 7200RPM HDD</li>
    <li><?php echo ms("Motherboard", "Emaplaat");?>: Asus TUF GAMING B550-PLUS</li>
    <li><?php echo ms("Motherboard I/O", "Emaplaadi pordid");?>: 1xUSB3 10Gbps Type-A, 1xUSB3 Type-C, 4xUSB3 Type-A, 2xUSB2 Type-A, 2.5Gbps Ethernet, HDMI, DP, 3.5mm audio, SPDIF <?php echo ms("output", "väljund");?></li>
    <li><?php echo ms("Video card", "Videokaart");?>: ASRock AMD RX7600 8GB OC</li>
    <li><?php echo ms("Power supply", "Toiteplokk");?>: Chieftec ELP-700S (700W, 200-240V)</li>
    <li><?php echo ms("Processor cooling", "Protsessori jahutus");?>: Pure Rock Slim 2</li>
    <li><?php echo ms("Misc. cooling", "Muu jahutus");?>: 1 <?php echo ms("unknown PWM fan", "tundmatu PWM ventilaator");?></li>
    <li><?php echo ms("Internal card reader", "Sisemine kaardilugeja");?> (<?php echo ms("chip", "kiipkaart");?>+SD/MMC+CF/MD+MS+xD)</li>
    <li><?php echo ms("Disc writer", "Plaadi kirjutaja");?>: Lite-ON DVD&#177;RW 48x</li>
    <li><?php echo ms("Monitors", "Monitorid");?>: 1x1920x1080@144Hz VA + 2x1280x1024@75Hz TN</li>
    <li><?php echo ms("Front panel I/O", "Esipaneeli pordid");?>: 2xUSB3+2x3.5mm HD audio</li>
    <li><?php echo ms("Case", "Korpus");?>: Ordi Samba 9th Gen ATX <?php echo ms("Mid-tower", "Miditorn");?></li>
    <li><?php echo ms("Modified UEFI boot logo", "Modifitseeritud UEFI buutimislogo");?>: <?= ms("Yes", "Jah"); ?> (<a href="/mas_db/images/BIOS_logo3.png"><?= ms("preview", "eelvaade") ?></a>)</li>
</ul>
<h3><?php echo ms("Laptop", "Sülearvuti");?></h3>
<ul>
    <li><?php echo ms("Processor", "Protsessor");?>: AMD Ryzen 7 5700U (8t/16l)</li>
    <li><?php echo ms("Memory", "Mälu");?>: 16GB (8GB plaadil + 8GB SO-DIMM)</li>
    <li><?php echo ms("Storage", "Andmekandja");?>: 512GB M.2 NVMe SSD</li>
    <li><?php echo ms("Motherboard/model", "Emaplaat/mudel");?>: Asus Vivobook 15 OLED (M513UA)</li>
    <li><?php echo ms("Video processing: Integrated", "Videoprotsessor: Integreeritud");?> AMD Radeon APU</li>
    <li><?php echo ms("I/O", "Pordid");?>: USB3 Type-A, USB3 Type-C, 2xUSB2 Type-A, HDMI, 3.5mm audio, 65W <?php echo ms("power", "toide");?>, Micro-SD</li>
    <li><?php echo ms("Battery", "Aku");?>: 42WH</li>
    <li><?php echo ms("Display", "Ekraan");?>: 1x1920x1080@60Hz OLED</li>
    <li><?php echo ms("Modified UEFI boot logo", "Modifitseeritud UEFI buutimislogo");?>: <?= ms("No", "Ei"); ?></li>
</ul>
